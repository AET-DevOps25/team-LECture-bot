...
class FlashcardPipeline:
    ...
    def _retrieve_context(self, request: models.FlashcardRequest) -> str:
        """
    ...
    def _generate_flashcards_for_context(self, context: str) -> list[models.Flashcard]:
            ...
            if isinstance(flashcard_data, list):
                for item in flashcard_data:
                    if isinstance(item, dict) and "question" in item and "answer" in item:
                        validated_flashcards.append(models.Flashcard(**item))
    ...
    def process_request(self, request: models.FlashcardRequest) -> models.FlashcardResponse:
        """
        ...
        all_results: list[models.FlashcardsForDocument] = []
        ...
        for doc_id in document_ids_to_process:
            ...
            if flashcards:
                all_results.append(
                    models.FlashcardsForDocument(document_id=doc_id, flashcards=flashcards)
                )
            else:
                print(f"No valid flashcards models for document {doc_id}.")

        return models.FlashcardResponse(course_space_id=request.course_space_id, flashcards=all_results)
